...
taxdumpMerge <- function(a, b, linconf = "b"){
  ...
  ## Loop over nodes (= taxonomic concepts) in 'b'
  ## Nodes in 'a' that have no corresponding node in 'b',
  ...
  for (i in BID){
    ...
    if (taxdump_isRoot(b, taxon)){
      next
    }
    save(a, b, i, file = "user_data/DEV_taxdumpMerge2.rda")
    a <- taxdumpResolveNode(x = a, y = b, taxon = taxon)
    ...
    #   if (length(setdiff(bbb$taxon, aaa$taxon))){
    #     stop("implement me!")
    #   }
    ...
    #     if (length(syns)){
    #       ## check if synonyms are present in b
    #       if (syns %in% ab$taxon_b){
    #         if (ab$status_b[ab$taxon_b %in% syns] == "scientific name"){
    ...
    #         }
    #       } else {
    #         ## No element of 'syns' is element of b: status of 'syns' changes to
    #         ## 'synonym'
    #         a$status[a$taxon == syns] <- "synonym"
    #       }
    #       
    #     }
    #     
