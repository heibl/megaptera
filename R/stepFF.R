...
stepF <- function(x){
  ...
  if (!x@update){
    ...
    dbRemoveTable(conn, msa.seq.tab)
  } 
  ...
  ## Create table for sequences meta data
  ## ------------------------------------
  slog(paste0("\nCreating table '", msa.tab, "' ... "), 
       file = logfile)
  ...
  ## --------------------------
  slog(paste0("\nCreating table '", msa.seq.tab, "' ... "), 
       file = logfile)
  SQL <- paste0(msa.seq.tab, "_pk")
  SQL <- paste("CREATE TABLE", msa.seq.tab, 
               "(locus character varying NOT NULL,", 
               "taxon character varying NOT NULL,",
               "nuc character(1) NOT NULL,",
               "pos integer NOT NULL,",
               "reliability real,",
               "CONSTRAINT", SQL, "PRIMARY KEY (locus, taxon, pos))")
  dbSendQuery(conn, SQL)
  slog("done", file = logfile)
  # }
